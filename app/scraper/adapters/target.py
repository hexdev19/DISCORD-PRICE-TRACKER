...
class TargetAdapter:
    domain_match = re.compile(r"^(?:www\.)?target\.com$")
    needs_browser = False
    ...
    async def extract(
        ...
        title = _first_text(tree, '//h1[@data-test="product-title"]') or _first_text(
            tree, '//h1'
        )
        price_text = _first_text(tree, '//*[@data-test="product-price"]')
        price = parse_price(price_text)
        currency = parse_currency(price_text, region_hint="US") or "USD"
        avail = _first_text(tree, '//*[@data-test="shippingATCButton"]')
        ...
        return ScrapeResult(
            ...
            region_hint="US",
            raw_fingerprint={"matched": "target-dom"},
        )
