...
def test_watch_list_empty_state() -> None:
    ...
    assert embed["footer"]["text"].startswith("page 1/1")


def test_watch_list_renders_rows() -> None:
    embed = embed_builder.watch_list([(_watch(), _product())], page=1, pages=1, total=1, cap=25)
    assert "`ABCD1234`" in embed["description"]
    assert "99.95" in embed["description"]


def test_watch_list_marks_paused() -> None:
    from datetime import datetime

    embed = embed_builder.watch_list(
        [(_watch(paused_at=datetime(2026, 1, 1)), _product())],
        page=1,
        pages=1,
        total=1,
        cap=25,
